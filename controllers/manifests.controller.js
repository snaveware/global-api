    ...
    try {
      if (!req.body || !req.body.update) {
        ...
        RequestHandler.throwError(
          400,
          " You need to provide the manifest's id "
        )();
      }
      ...
      if (update.status) {
        const shipmentsUpdation = await Shipments.update(
          { manifestId: id },
        ...
        if (shipmentsUpdation.error) {
          RequestHandler.throwError(
            500,
            "could not update the status of shipments in this manifest"
          )();
        }
      }
  ...
  static async generatePdf(req, res) {
    try {
      ...
      if (manifest.error || shipments.error) {
        RequestHandler.throwError(
          400,
          "could not retrieve either the manifest or the  shipments in that manifest "
        )();
      }
      ...
      const doc = new ManifestPdf(manifest.manifest, shipments.shipments, {
        margin: 50,
        size: "letter",
        layout: "landscape",
      });
      ...
      doc.pipe(
        fs.createWriteStream(
          `${appRoot}/pdfs/manifest_${req.params.manifestId}.pdf`
        )
      );
      doc.end();
